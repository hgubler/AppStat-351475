...
Note that having a non-zero coefficient for the $P$ (sar1 = 0.0079) means that the prediction for $X_t$ depends linearly on the value of $X_{t-12}$, so we observe a yearly seasonality in our time series (but not a very strong one as the corresponding coefficient is close to 0).
...
$$
where $\Sigma$ will allow for a dependency structure between observations (the observations of our temperature anomalies time series are obviously not independent). We specify the dependency structure $\Sigma$ through our SARIMA model of the time series, where we use the `corARMA` function from the `nlme` package to estimate $\Sigma$ based on the parameters $p$ and $q$ of our SARIMA model. The predictors in our model are `time` and the factor variable `month`. Furthermore we add an exponential dependence on time in our model and perform an anova type two test to test whether we have an exponential dependence on time or not. 
...
We see that the anova table (\@ref(tab:expdep)) suggests that there is no exponential dependence on time and we therefore refute the hypothesis of an exponential trend. Furthermore, the table suggests that a linear dependence on time seems important. 
...
According to the anova table (\@ref(tab:quaddep)), a quadratic dependence on time is significant.
