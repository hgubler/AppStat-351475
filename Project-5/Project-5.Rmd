...
Once we defined the hyperparameters of the SARIMA model, the parameters are estimated by Gaussian MLE. In the following sections, we allow all our SAMIRA models to include a drift parameter, which acts like an intercept.
...
The normal QQ-plot looks fine and therefore we may assume that the residuals of the SARIMA(2,1,3,1,0,0) model are approximately normal. Therefore we keep this model to build forecasts in the next section. In Table (\@ref(tab:coeftable)) we look at the coefficient of our final model.
...
Note that having a non-zero coefficient for the $P$ (0.0079) means that the prediction for $X_t$ depends linearly on the value of $X_{t-12}$, so we observe a yearly seasonality in our time series (but not a very strong one as the corresponding coefficient is close to 0).
...
In this report, we use a SARIMA model for a time series with temperature anomalies from 1950 to 2022. This model allows us to produce forecasts for the temperature development in the future. We detected a light seasonal effect in the temperature anomalies that can possibly be explained with the fact that most of the earth's landmass is in the northern hemisphere and hence anomalies in the summer behave differently than anomalies in the winter.

Furthermore, by using time series regression, we were able to refute the hypothesis of an exponential trend on time and suggested that a quadratic trend might be a more plausible scenario. However, these forecasts should be taken with caution as it is obviously extremely difficult to predict the form of the future global warming as it strongly depends on factors as political policies as well as natural feedback mechanisms.
