...
# function for negative log likelihood for the binned bootstrapped data to optimize in next step
negloglikelihood_boot = function(par) {
  logli = 0 # start at 0 and sum op at each datapoint
  mu1 = par[1]
  mu2 = par[2]
  sigma1 = par[3]
  ...
  tau = par[5]
  for (j in 1:length(data$X)) {
    logli = logli + (binned_bootstrap_data[j, i]) * log((cdfbilognorm(data$endpoint[j], mu1, mu2, sigma1, sigma2, tau) - 
                       cdfbilognorm(data$startpoint[j], mu1, mu2, sigma1, sigma2, tau)))
  }
  return(-logli) #return the negative log likelihood
}
...
for (i in 1:n_bootstrap) {
  out = optim(estim_boot[, i], negloglikelihood_boot, method="Nelder-Mead")
