...
4. Optimize the log likelihood of the binned, bootstrapped data.
...
ndata_boot = 705044 # number of data in bootstrap sample
...
In this report, we were able to find statistical arguments to show that the snowflake diameters follow a bi-log-normal distribution. In addition to that, using the EM algorithm and optimization of the binned log likelihood, we were able to find a set of parameter estimates such that our parametrized bi-log-normal model describes the data very well. These estimates can be found in Table (\@ref(tab:paramtable)) and below an instruction on how to simulate new snowflake parameters using these estimates.

The steps in this report also show that even if we only have access to binned data there are possibilities to find an appropriate statistical model for the data. However this requires some extra steps and we would always prefer non-binned data as it provides more information.