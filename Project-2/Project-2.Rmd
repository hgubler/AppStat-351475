...
# model
library(knitr)
kable(anova2, caption = "Type II Anova table for a logistic regression model including all predictors from the data.")
```
...
anovaquad_table[1, 3:5] = c("", "", "") # remove NA values from table
...
# Create table
kable(anovaquad_table, caption = "Anova Model-Submodel table to test for quadratic dependence on page_values")
```

The Anova test suggests us to keep the quadratic dependency on page_values, so we proceed with this model. We can check if the residuals plot for the predictor page_value now looks better.
...
```
 ...
 The residuals plot already looks a bit better but we can still observe some slight non-linearity. So we can also add a cubic term of page_values to the model. As before, we use the model-submodel Anova table to compare the model with cubic dependency on page_values and without cubic dependency on page_values (but still including quadratic dependency on page_values.)
```{r anovacube}
# fit a model with the significant predictors from before together with quadratic
# dependence on page_values
...
anovacube_table[1, 3:5] = c("", "", "") # remove NA values from table
# Add row names to the data frame
row.names(anovacube_table) <- c("Without cubic dependence on page_values",
                                  "With cubic dependence on page_values")
# Create table
kable(anovacube_table, caption = "Anova Model-Submodel table to test for quadratic dependence on page_values")
```

Again, the Anova table suggests us to keep the cubic dependency on page_values. Also we again look at the residuals vs predictor plot for page_values.
```{r pagevalueresidcube, fig.align='center', out.width="50%", fig.cap="Residuals vs the predictor page_values after including a cubic dependency on page_values into the model."}
dataplot = data.frame(x = data$page_values, y = gm_sig_cube$residuals)
ggplot(aes(x = data$page_values, y = gm_sig_cube$residuals), data = dataplot) +
  geom_point() + 
  geom_smooth() +
  ylab("Residuals") +
  xlab("Predictor Value") +
  ggtitle("Residuals vs Predictor")
```
We now almost reached the desirable flat line for the residuals plot. We won't add any more higher order terms for page_values for two reasons. First, we should not overinterpret the residuals plots for the logistic regression (since FWL not hold?) and secondly adding too many terms of higher order could lead to overfit.
