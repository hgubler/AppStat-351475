...
It is a hard task to build a statistical model when a dataset with a lot of variables is available, since there is a huge number of different combinations, transformations and interactions between variables. In this report, we carried out a possible procedure to build such a model with statistical reasoning, however a different dataset can require a different procedure. Also it would have been very helpful to have some deeper knowledge about the domain, since this would give some intuition on what variables are important or what transformations/interactions could have been useful.
