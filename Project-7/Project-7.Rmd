...
2. The observations from group 1 should have approximately the same *standard deviation* as the observations from group 2. The standard deviation in a group describes how spread out the set of blood glucose numbers is. In our case, we assume the standard deviation in both groups to be 15 mg/dl (a typical value for the standard deviation in blood glucose).
...
So when we take the significance level 0.05, a power level of 80% and a standard deviation of 15 mg/dl, we can calculate that we need 29 number of subjects in each group (group 1 and group 2).

Note that however if we change the standard deviation of the blood glucose, our result will change. As a higher standard deviation means that our datapoints are more spread out, we need a larger number of subjects in our groups if we keep the same significance level and power level.

The same goes for the significance level. If we want to lower the significance level (the probability of rejecting the Null Hypothesis $H_0$ when it is actually true), we need more subjects if we keep the same power level and standard deviation.

Similarly if we want to use a higher power level (the probability of correctly rejecting the Null Hypothesis $H_0$ when $H_1$ is true), we need more subjects if we keep the same significance level and standard deviation.
...
In this report we showed that, when using a typical standard deviation of blood glucose of 15 mg/dl and the by clinical dietitians widely used significance level of 5% and power level of 80%, we need 29 subjects per group to test the Null Hypothesis $H_0$ vs the Alternative Hypothesis $H_1$. However if we change those standard values for standard deviations, significance level and power level, the number of subjects needed per group changes as seen in Figure (\@ref(fig:samplesizeplot)).